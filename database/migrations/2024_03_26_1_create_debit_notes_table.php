...
class CreateDebitNotesTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('debit_notes', function (Blueprint $table) {
            ...
            // Foreign keys
            $table->foreign('company_id', 'FK_debit_notes_companies_id')
                ->references('company_id')->on('companies')
                ->onUpdate('restrict')->onDelete('restrict');
            $table->foreign('fn_id', 'FK_debit_notes_financial_years')
                ->references('fn_id')->on('financial_years')
                ->onUpdate('restrict')->onDelete('restrict');
            $table->foreign('credit_account_id', 'FK_debit_notes_parties')
                ->references('pid')->on('parties')
                ->onUpdate('restrict')->onDelete('restrict');
            $table->foreign('debit_account_id', 'FK_debit_notes_parties_2')
                ->references('pid')->on('parties')
                ->onUpdate('restrict')->onDelete('restrict');
            $table->foreign('uid', 'FK_debit_notes_users')
