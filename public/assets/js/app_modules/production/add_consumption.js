...
const Consumption = function () {
    const voucherType = 'consumptions';
    const baseUrl = base_url;
    const domElement = {
        consumptionHiddenId: $('#consumptionHiddenId'),
    ...
    var getConsumptionById = async function (consumptionId) {
        const response = await makeAjaxRequest('GET', `${baseUrl}/consumption/getConsumptionById`, {
    ...
    var populateData = function (data) {
        $('#consumptionHiddenId').val(data.id);
        ...
        $.each(data.consumption_detail, function (index, elem) {
            appendToTable(
    ...
    var getSaveObject = function () {
        ...
        consumption.id = $('#consumptionHiddenId').val();
        consumption.vrdate = $('#current_date').val();
        consumption.chk_date = $('#chk_date').val();
        consumption.received_by = $('#receivedBy').val();
        consumption.remarks = $('#remarks').val();

        $('#consumptionTable').find('tbody tr').each(function () {
            const row = $(this);
            ...
            consumptionDetail.push(gridItemDetail);
        });
        const data = {};
        data.consumption = consumption;
        data.consumptionDetail = consumptionDetail;
        data.id = $('#consumptionHiddenId').val();
        return data;
    };
    ...
    var deleteVoucher = async function (vrnoa) {
        general.disableSave();
        const response = await makeAjaxRequest('delete', `${baseUrl}/consumption/delete`,
    ...
    var calculateLowerTotal = function () {
        ...
        $("#consumptionTable").find("tbody tr").each(function (index, elem) {
    ...
    var resetFields = function () {
        $('.inputerror').removeClass('inputerror');
        const resetArray = [
            { selector: 'consumptionHiddenId', options: { disabled: true } },
            'current_date',
    ...
    const getConsumptionDataTable = (consumptionId = 0, fromDate = "", toDate = "") => {
        ...
        stockTransferDataTable = $("#stockTransferDataTable").DataTable({
            ...
            serverSide: true,
            ajax: {
                url: `${baseUrl}/consumption/getConsumptionDataTable`,
                type: 'GET',
                data: { 'consumptionId': consumptionId, fromDate: fromDate, toDate: toDate },
            ...
            columns: [
                ...
                },
                {
                    data: "vrnoa",
                    name: "vrnoa",
                    className: "text-left consumptionVoucher"
                },
                {
                    data: "vrdate",
                    name: "vrdate",
                    ...
                    render: function (data, type, row) {
						...
						<div class="btn-group dropleft">
							...
							<div class="dropdown-menu">
								<a class="dropdown-item btnEditPrevVoucher" data-consumption_id="${row.id}" href="#"><i class='fa fa-edit'></i> Edit Voucher</a>
								<a class="dropdown-item btnPrint"  data-consumption_id="${row.id}" href="#"><i class='fa fa-print'></i> Print Voucher</a>
								<a class="dropdown-item btnDelete" data-consumption_id="${row.id}" href="#"><i class='fa fa-trash'></i> Delete Voucher</a>
...
`;
    ...
    const appendToTable =
        (
          ...
          <tr class="odd:bg-white even:bg-slate-50">
            ...
            <td class='py-1 px-1 text-md align-middle text-right cost'>${parseNumber(cost).toFixed(RATE_ROUNDING)}</td>
            <td class='py-1 px-1 text-md align-middle text-right gAmount'>${parseNumber(amount).toFixed(AMOUNT_ROUNDING)}</td>
        ...
        `;
            $(row).appendTo('#consumptionTable');
            getTableSerialNumber('#consumptionTable');
        };
    ...
    async function validateInventory(itemId, stockKeepingMethodId, colorCodeId, departmentId, qty, weight, currentDate) {
        const inventoryManager = new InventoryManager({ "voucherType": voucherType, "voucherId": $('#consumptionHiddenId').val() });
    ...
    return {
        ...
        bindUI: function () {
            ...
            $('#consumptionTable').on('click', '.btnRowRemove', function (e) {
                e.preventDefault();
            ...
            $('#consumptionTable').on('click', '.btnRowEdit', function (e) {
                e.preventDefault();
                const row = $(this).closest('tr');
                ...
                const stockInfo = new StockInformation(editStockKeepingMethodId, { isSale: true, voucherType: voucherType, voucherId: $('#consumptionHiddenId').val() });
                stockInfo.getStockInformation(parseNumber(editItemId), $('#current_date').val());
                row.remove();
                calculateLowerTotal();
            });

            $('#consumptionSyncAlt').on('click', function (e) {
                e.preventDefault();
                $('#fromDate').datepicker('update', new Date());
                $('#toDate').datepicker('update', new Date());
                getConsumptionDataTable();
            });
            $('#consumptionFilter').on('click', function (e) {
                e.preventDefault();
                const fromDate = $('#fromDate').val();
                const toDate = $('#toDate').val();
                getConsumptionDataTable("", fromDate, toDate);
            });
            ...
            $(document.body).on('click', '.btnEditPrevVoucher', function (event) {
                event.preventDefault();
                const consumptionId = parseNumber($(this).data('consumption_id'));
                getConsumptionById(consumptionId);
            ...
            $(document.body).on('click', '.btnPrint', function (event) {
                ...
                printVoucher(consumptionId, settingPrintDefault, 'lg', '');
            });
            ...
            $(document.body).on('click', '.btnPrintASEmail', function (event) {
                ...
                const consumptionId = parseNumber($(this).data('consumption_id'));
                const settingPrintDefault = $('#setting_print_default').val();
                getSendMail(consumptionId, settingPrintDefault, 'lg', '', true);
            });
        ...
        initSave: function () {
            ...
            if (!validateSaveFlag) {
                var rowsCount = $('#consumptionTable').find('tbody tr').length;
                if (rowsCount > 0) {
                    const saveObj = getSaveObject();
                    const data = { 'consumption': JSON.stringify(saveObj.consumption), 'consumptionDetail': JSON.stringify(saveObj.consumptionDetail), 'id': saveObj.id, 'chk_date': $('#chk_date').val(), 'vrdate': $('#current_date').val() };
