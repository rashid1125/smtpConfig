import { appendSelect2ValueIfDataExists, clearValueAndText, padNumberWithLeadingCharacters, parseNumber, resetSelect2Option, updateDatepickerWithFormattedDate } from '../commonFunctions/CommonFunction.js';
import { dropdownOptions } from "../../../../js/components/GlobalUrl.js";
import DynamicOption from "../../../../js/components/DynamicOption.js";

var GeneralAccounts = function () {
	const saveGeneralAccounts= `${base_url}/account/generalaccount/saveGeneralAccounts`;
	const getGeneralAccountsById = `${base_url}/account/generalaccount/getByGeneralAccountsId`;
	const settingConfigurGeneralAccountsLevel3 = $('#settingConfigurGeneralAccountsLevel3').val();
    var settings = {

		// basic information section

		txtGeneralAccountsIdHidden: $('#txtGeneralAccountsIdHidden'),
		txtName: $('#txtName'),
		txtUName: $('#txtUName'),
		txtPid: $("#txtPid"),
		txtLevel3: $('#txtLevel3'),
    ...
    var save = async function (accountObj) {
		...
		try {
			const response = await new Promise((resolve, reject) => {
				$.ajax({
					url: `${saveGeneralAccounts}`,
					type: 'POST',
					data: { 'generalAccountsdetail': accountObj },
    ...
    var getSaveObject = function () {
		const obj = {
			pid: $.trim($(settings.txtGeneralAccountsIdHidden).val()),
	...
	var populateData = function (elem) {
		$('#txtGeneralAccountsIdHidden').val(elem.pid);
		// Check if the 'active' property is truthy (1 for active, 0 for inactive)
		const active = parseNumber(elem.active) === 1;
		// Set the switch button state based on the 'active' property
		...
		// Trigger the change event after updating txtLevel3
    ...
    return {
		...
		bindUI: function () {
			...
			$('#txtLevel3').val(settingConfigurGeneralAccountsLevel3).trigger('change');
			...
			$(settings.btnSave).on('click', function (e) {
				if ($('#txtGeneralAccountsIdHidden').val() == 'edit' && $('.btnSave').data('updatebtn') == 0) {
					alert('Sorry! you have not update rights..........');
				} else if ($('#txtGeneralAccountsIdHidden').val() == 'new' && $('.btnSave').data('insertbtn') == 0) {
			...
			$('body').on('click', '.btn-edit-generalaccounts', function (e) {
...
var generalaccounts= new GeneralAccounts();
generalaccounts.init();
$(function () {
