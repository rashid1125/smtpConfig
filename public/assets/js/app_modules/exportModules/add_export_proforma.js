...
var ExportProforma = function () {

    const voucherType = 'export_proformas';
    ...
    const saverRequest = new SaverRequest(
        ...
        general,
        {
            requestedUrl: 'exportProforma/save',
            requestType: 'POST',
            isConfirmed: true,
    ...
    var getSaveObject = function () {
        ...
        const exportProformaDetail = [];

        exportProforma.id = $('#exportProformaIdHidden').val();
        exportProforma.vrdate = $('#current_date').val();
        exportProforma.order_valid_till = $('#due_date').val();
        exportProforma.chk_date = $('#chk_date').val();
        exportProforma.party_id = $('#accountDropdown').val();
        exportProforma.sale_officer_id = $('#saleOfficerDropdown').val();
        exportProforma.export_quotation_id = $('#exportQuotationIdHidden').val();
        exportProforma.currency_id = $('#gridItemCurrencyDropdown').val();
        exportProforma.port_of_discharge_id = $('#portOfDischargeDropdown').val();
        exportProforma.customer_name = $('#customerName').val();
        exportProforma.customer_mobile = $('#customerMobile').val();
        exportProforma.prepared_by = $('#receivers_list').val();
        exportProforma.commission_percentage = $('#saleCommissionPercentage').val();
        exportProforma.net_amount = parseNumber($('.gridItemTotalGrossAmount').text()) || 0;
        ...
        $('#purchase_table').find('tbody tr').each(function (index, elem) {
            ...
            exportProformaDetail.push(gridItemDetail);
        });
        const data = {};
        data.exportProforma = exportProforma;
        data.exportProformaDetail = exportProformaDetail;
        data.id = $('#exportProformaIdHidden').val();
        return data;
    };
    ...
    var deleteVoucher = async function (vrnoa) {
        general.disableSave();
        const response = await makeAjaxRequest('delete', `${base_url}/exportProforma/delete`,
    ...
    var resetFields = function () {
        const resetArray = [
            ...
            { selector: 'quotationNumber', options: { disabled: true } },
    ...
    let exportProformaViewList = undefined;
    const getExportProformaDataTable = (exportProformaId = 0, fromDate = "", toDate = "") => {
        if (typeof exportProformaViewList !== 'undefined') {
            exportProformaViewList.destroy();
            $('#exportProformaViewListTbody').empty();
        }
        exportProformaViewList = $("#exportProformaViewList").DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                url: `${base_url}/exportProforma/getExportProformaDataTable`,
                type: 'GET',
                data: { 'exportProformaId': exportProformaId, fromDate: fromDate, toDate: toDate },
            ...
            columns: [
                ...
                },
                {
                    data: "vrnoa",
                    name: "vrnoa",
                    className: "text-left exportProformaVoucher"
                },
                {
                    data: "vrdate",
                    name: "vrdate",
        ...
        exportProformaViewList.on('draw', function () {
    ...
    let pendingQuotationDataTable = undefined;
    const getPendingQuotationDataTable = () => {
        if (typeof pendingQuotationDataTable !== 'undefined') {
            pendingQuotationDataTable.destroy();
            $('#pendingQuotationDataTableTbody').empty();
        }
        pendingQuotationDataTable = $("#pendingQuotationDataTable").DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                url: `${base_url}/exportQuotation/getPendingQuotationDataTable`,
                type: 'GET',
                dataSrc: function (json) {
                    return json.data;
                }
            ...
            columns: [
                ...
                },
                {
                    data: "vrnoa",
                    name: "vrnoa",
                    className: "text-left quotationNumber"
                },
                {
                    data: "vrdate",
                    name: "vrdate",
                ...
                {
                    data: "order_valid_till",
                    name: "order_valid_till",
                    className: "text-left order_valid_till",
                    render: function (data, type, row) {
                        return getFormattedDate(data);
                    }
                },
                {
                    ...
                    orderable: false,
                    render: function (data, type, row) {
                        return `<button type="button" data-dismiss="modal" class="btn btn-sm btn-outline-primary group-hover:bg-blue-600 group-hover:border-blue-600 group-hover:text-white mr-2 mb-2 flex-1 populatePendingExportQuotation" data-vrnoa="${data.id}"><i class='fa fa-edit'></i></button>`;
                    }
                }
        ...
        pendingQuotationDataTable.on('draw', function () {
            $('[data-toggle="tooltip"]').tooltip();
        });
        $('#pendingQuotationDataTableLookUp').modal('show');
    };
    ...
    var getExportQuotationById = async function (exportQuotationId) {
        ...
        if (response.status == false && response.error !== "") {
            ...
            populateExportQuotationData(response.data);
        }
    };

    var populateExportQuotationData = function (data) {
        $('#quotationNumber').val(data.vrnoa);
    ...
    return {
        ...
        bindUI: function () {
            ...
            $('#exportProformaSyncAlt').on('click', function (e) {
                e.preventDefault();
                $('#fromDate').datepicker('update', new Date());
                $('#toDate').datepicker('update', new Date());
                getExportProformaDataTable();
            });
            $('#exportProformaFilter').on('click', function (e) {
                e.preventDefault();
                const fromDate = $('#fromDate').val();
                const toDate = $('#toDate').val();
                getExportProformaDataTable("", fromDate, toDate);
            });
            ...
            $('body').on('click', '.btnEditPrevVoucher', function (e) {
                e.preventDefault();
                var exportProformaId = parseNumber($(this).data('sale_order_id'));
                getExportProformaById(exportProformaId);
                $('a[href="#Main"]').trigger('click');
            });
            $('body').on('click', '.modal-lookup .populatePendingExportQuotation', async function (e) {
                e.preventDefault();
                const exportQuotationId = parseNumber($(this).data('vrnoa'));
                resetFields();
                await getExportQuotationById(exportQuotationId);
                calculateLowerTotal();
            });
            $('body').on('click', '#quotationButton', function (event) {
                getPendingQuotationDataTable();
            });
            ...
            $('body').on('click', '.btnPrint', function (e) {
                const exportProformaId = $(this).data('sale_order_id');
                ...
                if ($('#setting_print_default').val() == 5 || $('#setting_print_default').val() == 6) {
                    ...
                    Print_Voucher(exportProformaId, settingPrintDefault, 'lg', '');
                }
            });
            ...
            $('body').on('click', '.btnPrintA4WithHeader', function (e) {
                ...
                if ($('#setting_print_default').val() == 5 || $('#setting_print_default').val() == 6) {
                    ...
                    Print_Voucher(exportProformaId, 1, 'lg', "");
                }
            });
            $('body').on('click', '.btnPrintA4WithOutHeader', function (e) {
                ...
                if ($('#setting_print_default').val() == 5 || $('#setting_print_default').val() == 6) {
                    ...
                    Print_Voucher(exportProformaId, 2, 'lg', "");
                }
            });
            $('body').on('click', '.btnPrintB5WithHeader', function (e) {
                ...
                if ($('#setting_print_default').val() == 5 || $('#setting_print_default').val() == 6) {
                    ...
                    Print_Voucher(exportProformaId, 3, 'lg', "");
                }
            });
            $('body').on('click', '.btnPrintB5WithOutHeader', function (e) {
                ...
                if ($('#setting_print_default').val() == 5 || $('#setting_print_default').val() == 6) {
                    ...
                    Print_Voucher(exportProformaId, 4, 'lg', "");
                }
            });
            ...
            $('body').on('click', '.btnPrintASEmail', function (e) {
                ...
                const settingPrintDefault = $('#setting_print_default').val();
                e.preventDefault();
                getSendMail(exportProformaId, settingPrintDefault, 'lg', '', true);
            });
            ...
            $('body').on('click', '.btnDelete', function (e) {
                ...
                if (exportProformaId !== '') {
                    _getConfirmMessage('Warning!', 'Are you sure to delete this voucher', 'warning', function (result) {
                        if (result) {
                            deleteVoucher(exportProformaId);
                        }
                    });
                }
            });
            ...
            exportProforma.fetchRequestedVr();
        },
            ...
            if (!validateSaveFlag) {
                if (!validateDueDate) {
                    ...
                    if (rowsCount > 0) {
                        ...
                        const data = { 'exportProforma': JSON.stringify(saveObj.exportProforma), 'exportProformaDetail': JSON.stringify(saveObj.exportProformaDetail), 'id': saveObj.id, 'chk_date': $('#chk_date').val(), 'vrdate': $('#cur_date').val() };
...
const exportProforma = new ExportProforma();
exportProforma.init();
