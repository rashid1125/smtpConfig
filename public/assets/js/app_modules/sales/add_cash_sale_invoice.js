...
const CashSaleInvoice = function () {
    const voucherType = "cash_sale_invoices";
    ...
    const saverRequest = new SaverRequest(base_url, general, {
        requestedUrl      : "cashSaleInvoice/save",
        requestType       : "POST",
        isConfirmed       : true,
    ...
    var getCashSaleInvoiceById = async function (cashSaleInvoiceId) {
        const response = await makeAjaxRequest("GET", `${base_url}/cashSaleInvoice/getCashSaleInvoiceById`, {
            cashSaleInvoiceId : cashSaleInvoiceId
        });
        resetFields();
    ...
    var getSaveObject = function () {
        const cashSaleInvoice       = {};
        const cashSaleInvoiceDetail = [];

        cashSaleInvoice.id                     = $("#cashSaleInvoiceHiddenId").val();
        cashSaleInvoice.vrdate                 = $("#current_date").val();
        cashSaleInvoice.chk_date               = $("#chk_date").val();
        cashSaleInvoice.due_days               = $("#due_days").val();
        cashSaleInvoice.due_date               = $("#due_date").val();
        cashSaleInvoice.party_id               = $("#accountDropdown").val();
        cashSaleInvoice.sale_officer_id        = $("#saleOfficerDropdown").val();
        cashSaleInvoice.sale_order_id          = $("#saleOrderHiddenId").val();
        cashSaleInvoice.delivery_challan_id    = $("#deliveryChallanHiddenId").val();
        cashSaleInvoice.customer_name          = $("#customerName").val();
        cashSaleInvoice.customer_mobile        = $("#customerMobile").val();
        cashSaleInvoice.remarks                = $("#voucherRemarks").val();
        cashSaleInvoice.prepared_by            = $("#receivers_list").val();
        cashSaleInvoice.bilty_number           = $("#biltyNumber").val();
        cashSaleInvoice.bilty_date             = $("#biltyDate").val();
        cashSaleInvoice.transporter_id         = $("#transporterDropdown").val();
        cashSaleInvoice.freight_amount         = $("#freightAmount").val();
        cashSaleInvoice.freight_type_id        = $("#freightTypeDropdown").val();
        cashSaleInvoice.discount_percentage    = $("#txtDiscount").val();
        cashSaleInvoice.discount_amount        = $("#txtDiscAmount").val();
        cashSaleInvoice.expense_percentage     = $("#txtExpense").val();
        cashSaleInvoice.expense_amount         = $("#txtExpAmount").val();
        cashSaleInvoice.further_tax_percentage = $("#txtTax").val();
        cashSaleInvoice.further_tax_amount     = $("#txtTaxAmount").val();
        cashSaleInvoice.commission_percentage  = $("#saleCommissionPercentage").val();
        cashSaleInvoice.commission_amount      = $("#saleCommissionAmount").val();
        cashSaleInvoice.net_amount             = parseNumber($("#txtNetAmount").val()) || 0;
        ...
        $("#purchase_table").find("tbody tr").each(function (index, elem) {
            ...
            cashSaleInvoiceDetail.push(gridItemDetail);
        });
        const data                 = {};
        data.cashSaleInvoice       = cashSaleInvoice;
        data.cashSaleInvoiceDetail = cashSaleInvoiceDetail;
        data.id                    = $("#cashSaleInvoiceHiddenId").val();
        return data;
    };
    ...
    var deleteVoucher = async function (vrnoa) {
        general.disableSave();
        const response = await makeAjaxRequest("delete", `${base_url}/cashSaleInvoice/delete`, {
    ...
    let cashSaleInvoiceDataTable      = undefined;
    const getCashSaleInvoiceDataTable = (cashSaleInvoiceId = 0, fromDate = "", toDate = "") => {
        if (typeof cashSaleInvoiceDataTable !== "undefined") {
            cashSaleInvoiceDataTable.destroy();
            $("#cashSaleInvoiceDataTableTbody").empty();
        }
        cashSaleInvoiceDataTable = $("#cashSaleInvoiceDataTable").DataTable({
            processing : true,
            serverSide : true,
            ajax       : {
                url     : `${base_url}/cashSaleInvoice/getCashSaleInvoiceDataTable`,
                type    : "GET",
                data    : {
                    "cashSaleInvoiceId" : cashSaleInvoiceId,
                    fromDate            : fromDate,
...
`;
        ...
        cashSaleInvoiceDataTable.on("draw", function () {
    ...
    async function validateInventory(itemId, stockKeepingMethodId, colorCodeId, departmentId, qty, weight, currentDate) {
        const inventoryManager = new InventoryManager({
            ...
            "voucherId"   : $("#cashSaleInvoiceHiddenId").val()
        });
    ...
    return {
        ...
        bindUI : function () {
            ...
            $("#cashSaleInvoiceSyncAlt").on("click", function (e) {
                e.preventDefault();
            ...
            $("#saleInvoiceFilter").on("click", function (e) {
                ...
                getCashSaleInvoiceDataTable("", fromDate, toDate);
            });
            ...
            $("body").on("click", ".btnEditPrevVoucher", function (e) {
                e.preventDefault();
                const cashSaleInvoiceId = parseNumber($(this).data("cash_sale_invoice_id"));
                getCashSaleInvoiceById(cashSaleInvoiceId);
            ...
            $("body").on("click", ".btnPrint", function (e) {
                ...
                e.preventDefault();
                Print_Voucher(cashSaleInvoiceId, settingPrintDefault, "lg", "");
            });
            ...
            $("body").on("click", ".btnPrintASEmail", function (e) {
                ...
                const settingPrintDefault = $("#setting_print_default").val();
                e.preventDefault();
                getSendMail(cashSaleInvoiceId, settingPrintDefault, "lg", "", true);
            });
            ...
            cashSaleInvoice.fetchRequestedVr();
        },
        ...
        initSave         : function () {
            ...
            if (! validateSaveFlag) {
                ...
                if (rowsCount > 0) {
                    ...
                    const data    = {
                        "cashSaleInvoice"       : JSON.stringify(saveObj.cashSaleInvoice),
                        "cashSaleInvoiceDetail" : JSON.stringify(saveObj.cashSaleInvoiceDetail),
                        "id"                    : saveObj.id,
                        "chk_date"              : $("#chk_date").val(),
...
const cashSaleInvoice = new CashSaleInvoice();
