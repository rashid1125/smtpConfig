...
import { appendSelect2ValueIfDataExists, clearValueAndText, getCalculateDatesWithCurrentDate, getDateDifference, getItemRateTypeById, getValueIfDataExists, handlePercentageOrAmountInput, ifNull, isPositive, isPreviousBalance, parseNumber, selectLastOption, setFinancialYearDate, updateDatepickerWithFormattedDate, updateDateRangeCurrentDay, updateDateRangeToCurrentMonth, updateDateRangeToCurrentWeek, updateFormattedDate } from "../commonFunctions/CommonFunction.js";
...
const SaleInvoice = function () {
    const voucherType = "sale_invoices";
    ...
    var getSaleInvoiceById = async function (saleInvoiceId) {
        const response = await makeAjaxRequest("GET", `${base_url}/saleInvoice/getSaleInvoiceById`, {
    ...
    var populateData = function (data) {
        ...
        $("#saleInvoiceHiddenId").val(data.id);
        ...
        $("#saleOrderHiddenId").val(getValueIfDataExists(data, "sale_order.id", ""));

        $("#deliveryChallanVrnoa").val(getValueIfDataExists(data, "delivery_challan.vrnoa", ""));
        $("#deliveryChallanHiddenId").val(getValueIfDataExists(data, "delivery_challan.id", ""));
        ...
        $.each(data.sale_invoice_detail, function (index, elem) {
    ...
    var getSaveObject = function () {
        ...
        saleInvoice.id                     = $("#saleInvoiceHiddenId").val();
        saleInvoice.vrdate                 = $("#current_date").val();
        saleInvoice.chk_date               = $("#chk_date").val();
        saleInvoice.due_days               = $("#due_days").val();
        saleInvoice.due_date               = $("#due_date").val();
        saleInvoice.party_id               = $("#accountDropdown").val();
        saleInvoice.sale_officer_id        = $("#saleOfficerDropdown").val();
        saleInvoice.sale_order_id          = $("#saleOrderHiddenId").val();
        saleInvoice.delivery_challan_id    = $("#deliveryChallanHiddenId").val();
        saleInvoice.customer_name          = $("#customerName").val();
        saleInvoice.customer_mobile        = $("#customerMobile").val();
        saleInvoice.prepared_by            = $("#receivers_list").val();
        saleInvoice.bilty_number           = $("#biltyNumber").val();
        saleInvoice.bilty_date             = $("#biltyDate").val();
        saleInvoice.transporter_id         = $("#transporterDropdown").val();
        saleInvoice.freight_amount         = $("#freightAmount").val();
        saleInvoice.freight_type_id        = $("#freightTypeDropdown").val();
        saleInvoice.discount_percentage    = $("#txtDiscount").val();
        saleInvoice.discount_amount        = $("#txtDiscAmount").val();
        saleInvoice.expense_percentage     = $("#txtExpense").val();
        saleInvoice.expense_amount         = $("#txtExpAmount").val();
        saleInvoice.further_tax_percentage = $("#txtTax").val();
        saleInvoice.further_tax_amount     = $("#txtTaxAmount").val();
        saleInvoice.commission_percentage  = $("#saleCommissionPercentage").val();
        saleInvoice.commission_amount      = $("#saleCommissionAmount").val();
        saleInvoice.net_amount             = parseNumber($("#txtNetAmount").val()) || 0;
        ...
        $("#purchase_table").find("tbody tr").each(function (index, elem) {
            ...
            saleInvoiceDetail.push(gridItemDetail);
        });
        const data             = {};
        data.saleInvoice       = saleInvoice;
        data.saleInvoiceDetail = saleInvoiceDetail;
        data.id                = $("#saleInvoiceHiddenId").val();
        return data;
    };
    ...
    var deleteVoucher = async function (vrnoa) {
        general.disableSave();
        const response = await makeAjaxRequest("delete", `${base_url}/saleInvoice/delete`, {
    ...
    var resetFields = function () {
        $(".inputerror").removeClass("inputerror");
        const resetArray = [{
            selector : "saleInvoiceHiddenId",
            options  : { disabled : true }
        }, {
            selector : "saleOrderVrnoa",
            options  : { disabled : true }
        }, {
            selector : "saleOrderHiddenId",
            options  : { disabled : true }
        }, {
            selector : "deliveryChallanHiddenId",
            options  : { disabled : true }
        }, {
            selector : "deliveryChallanVrnoa",
            options  : { disabled : true }
    ...
    let saleInvoiceDataTable      = undefined;
    const getSaleInvoiceDataTable = (saleInvoiceId = 0, fromDate = "", toDate = "") => {
        if (typeof saleInvoiceDataTable !== "undefined") {
            saleInvoiceDataTable.destroy();
            $("#saleInvoiceDataTableTbody").empty();
        }
        saleInvoiceDataTable = $("#saleInvoiceDataTable").DataTable({
            processing : true,
            serverSide : true,
            ajax       : {
                url     : `${base_url}/saleInvoice/getSaleInvoiceDataTable`,
                type    : "GET",
                data    : {
                    "saleInvoiceId" : saleInvoiceId,
                    fromDate        : fromDate,
            ...
            columns    : [{
                ...
                data      : "delivery_challan.vrnoa",
                name      : "deliveryChallan.vrnoa",
                className : "deliveryChallanVrnoa",
                render    : function (data, type, row) {
                    return ifNull(data, "-");
                }
            }, {
                data      : "delivery_challan.vrdate",
                name      : "deliveryChallan.vrdate",
                className : "deliveryChallanVrdate",
                render    : function (data, type, row) {
                    if (data !== undefined) {
                        return updateFormattedDate(row.delivery_challan.vrdate);
                    }
                    return "-";
                }
            },
...
`;
        ...
        saleInvoiceDataTable.on("draw", function () {
            $("[data-toggle=\"tooltip\"]").tooltip();
        });
    };

    let pendingDeliveryChallanDataTable      = undefined;
    const getPendingDeliveryChallanDataTable = () => {
        if (typeof pendingDeliveryChallanDataTable !== "undefined") {
            pendingDeliveryChallanDataTable.destroy();
            $("#pendingDeliveryChallanDataTableTbody").empty();
        }
        pendingDeliveryChallanDataTable = $("#pendingDeliveryChallanDataTable").DataTable({
            processing : true,
            serverSide : true,
            ajax       : {
            ...
            columns    : [{
                ...
                orderable  : false,
                render     : function (data, type, row) {
                    return `<button type="button" class="btn btn-sm btn-outline-primary mr-2 mb-2 flex-1 getPendingDeliveryChallanById w-16 group-hover:bg-blue-600 group-hover:border-blue-600 group-hover:text-white align-middle" data-delivery_challan_id="${row.id}"><i class='fa fa-check'></i></button>`;
                }
            }
        ...
        // Reinitialize tooltips on table redraw
        pendingDeliveryChallanDataTable.on("draw", function () {
            $("[data-toggle=\"tooltip\"]").tooltip(); // For Bootstrap 4
            // $('[data-bs-toggle="tooltip"]').tooltip(); // For Bootstrap 5, if you are using it
        });
        $("#pendingDeliveryChallanModal").modal("show");
    };

    var getPendingDeliveryChallanById = async function (deliveryChallanId) {
        const response = await makeAjaxRequest("GET", `${base_url}/deliveryChallan/getPendingDeliveryChallanById`, {
        ...
        if (response.status == false && response.error !== "") {
            ...
            populatePendingDeliveryChallanData(response.data);
        }
    };

    var populatePendingDeliveryChallanData = async function (data) {
        ...
        $("#deliveryChallanVrnoa").val(getValueIfDataExists(data, "vrnoa", ""));
        $("#deliveryChallanHiddenId").val(getValueIfDataExists(data, "id", ""));
        ...
        $("#biltyNumber").val(data.bilty_number);
        updateDatepickerWithFormattedDate("biltyDate", data.bilty_date);
        $("#due_days").val(getValueIfDataExists(data.party, "credit_days", ""));
        appendSelect2ValueIfDataExists("transporterDropdown", "transporter", "transporter_id", "name", data);
    ...
    return {
        ...
        bindUI : function () {
            ...
            $("#saleInvoiceSyncAlt").on("click", function (e) {
                e.preventDefault();
                $("#fromDate").datepicker("update", new Date());
                $("#toDate").datepicker("update", new Date());
                getSaleInvoiceDataTable();
            });
            $("#saleInvoiceFilter").on("click", function (e) {
                e.preventDefault();
                const fromDate = $("#fromDate").val();
                const toDate   = $("#toDate").val();
                getSaleInvoiceDataTable("", fromDate, toDate);
            });
            ...
            $(document).on("click", "#searchDeliveryChallanVrnoa", async function (e) {
                e.preventDefault();
                getPendingDeliveryChallanDataTable();
            });
            $("body").on("click", "#pendingDeliveryChallanModal .getPendingDeliveryChallanById", async function (e) {
                e.preventDefault();
                $("#pendingDeliveryChallanModal").modal("hide");
                const deliveryChallanById = $(this).data("delivery_challan_id");
                await getPendingDeliveryChallanById(deliveryChallanById);
            });
            $("body").on("click", ".btnEditPrevVoucher", function (e) {
                e.preventDefault();
                const saleInvoiceId = parseNumber($(this).data("sale_invoice_id"));
            ...
            $("body").on("click", ".btnPrint", function (e) {
                ...
                e.preventDefault();
                Print_Voucher(saleInvoiceId, settingPrintDefault, "lg", "");
            });
            ...
            $("body").on("click", ".btnPrintA4WithHeader", function (e) {
                ...
                if ($("#setting_print_default").val() == 5 || $("#setting_print_default").val() == 6) {
                    ...
                    Print_Voucher(saleInvoiceId, 1, "lg", "");
                }
            });
            $("body").on("click", ".btnPrintA4WithOutHeader", function (e) {
                ...
                if ($("#setting_print_default").val() == 5 || $("#setting_print_default").val() == 6) {
                    ...
                    Print_Voucher(saleInvoiceId, 2, "lg", "");
                }
            });
            $("body").on("click", ".btnPrintB5WithHeader", function (e) {
                ...
                if ($("#setting_print_default").val() == 5 || $("#setting_print_default").val() == 6) {
                    ...
                    Print_Voucher(saleInvoiceId, 3, "lg", "");
                }
            });
            $("body").on("click", ".btnPrintB5WithOutHeader", function (e) {
                ...
                if ($("#setting_print_default").val() == 5 || $("#setting_print_default").val() == 6) {
                    ...
                    Print_Voucher(saleInvoiceId, 4, "lg", "");
                }
            });
            ...
            $("body").on("click", ".btnPrintASEmail", function (e) {
                ...
                const settingPrintDefault = $("#setting_print_default").val();
                e.preventDefault();
                getSendMail(saleInvoiceId, settingPrintDefault, "lg", "", true);
            });
        ...
        initSave         : function () {
            ...
            if (! validateSaveFlag) {
                ...
                if (rowsCount > 0) {
                    ...
                    const data    = {
                        "saleInvoice"       : JSON.stringify(saveObj.saleInvoice),
                        "saleInvoiceDetail" : JSON.stringify(saveObj.saleInvoiceDetail),
                        "id"                : saveObj.id,
                        "chk_date"          : $("#chk_date").val(),
