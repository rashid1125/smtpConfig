...
const ReturnInward = function () {
    ...
    const saverRequest = new SaverRequest(
        ...
        general,
        {
            requestedUrl: 'returnInward/save',
            requestType: 'POST',
            isConfirmed: true,
    ...
    var Print_Voucher = function (vrnoa, paperSize, printSize, wrate = 0, isSendEmail = false) {
        try {
            ...
            const __url = baseUrl + '/doc/getPrintVoucherPDF/?etype=' + __etype + '&vrnoa=' + __vrnoa + '&pre_bal_print=' + __pre_bal_print + '&paperSize=' + paperSize + '&printSize=' + printSize + '&wrate=' + (wrate ? wrate : 0) + '&language_id=' + __lang;
    ...
    var getReturnInwardById = async function (returnInwardId) {
        const response = await makeAjaxRequest('GET', `${baseUrl}/returnInward/getReturnInwardById`, {
    ...
    var populateData = function (data) {
        ...
        const tableAppended = new TableRowAppended('#purchase_table', { ...propsForTable, isUnitRate: false });
    ...
    var getSaveObject = function () {
        const returnInward = {};
        const returnInwardDetail = [];

        returnInward.id = $('#returnInwardHiddenId').val();
        returnInward.vrdate = $('#current_date').val();
        returnInward.chk_date = $('#chk_date').val();
        returnInward.party_id = $('#accountDropdown').val();
        returnInward.sale_officer_id = $('#saleOfficerDropdown').val();
        returnInward.sale_order_id = $('#saleOrderIdHidden').val();
        returnInward.customer_name = $('#customerName').val();
        returnInward.customer_mobile = $('#customerMobile').val();
        returnInward.prepared_by = $('#receivers_list').val();
        returnInward.vehicle_number = $('#vehicleNumber').val();
        returnInward.driver_name = $('#driverName').val();
        returnInward.remarks = $('#voucherRemarks').val();
        returnInward.bilty_number = $('#biltyNumber').val();
        returnInward.bilty_date = $('#biltyDate').val();
        returnInward.transporter_id = $('#transporterDropdown').val();
        returnInward.freight_amount = $('#freightAmount').val();
        returnInward.freight_type_id = $('#freightTypeDropdown').val();
        returnInward.discount_percentage = $('#txtDiscount').val();
        returnInward.discount_amount = $('#txtDiscAmount').val();
        returnInward.expense_percentage = $('#txtExpense').val();
        returnInward.expense_amount = $('#txtExpAmount').val();
        returnInward.further_tax_percentage = $('#txtTax').val();
        returnInward.further_tax_amount = $('#txtTaxAmount').val();
        returnInward.commission_percentage = $('#saleCommissionPercentage').val();
        returnInward.net_amount = parseNumber($('#txtNetAmount').val()) || 0;
        ...
        $('#purchase_table').find('tbody tr').each(function (index, elem) {
            ...
            returnInwardDetail.push(gridItemDetail);
        });
        const data = {};
        data.returnInward = returnInward;
        data.returnInwardDetail = returnInwardDetail;
        data.id = $('#returnInwardHiddenId').val();
        return data;
    };
    ...
    var deleteVoucher = async function (vrnoa) {
        general.disableSave();
        const response = await makeAjaxRequest('delete', `${baseUrl}/returnInward/delete`,
    ...
    var resetFields = function () {
        $('.inputerror').removeClass('inputerror');
        const resetArray = [
            { selector: 'returnInwardHiddenId', options: { disabled: true } },
            { selector: 'saleOrderIdHidden', options: { disabled: true } },
            { selector: 'saleOrderVrnoa', options: { disabled: true } },
            'accountDropdown',
            'current_date',
            'chk_date',
            'saleOfficerDropdown',
            ...
            'gridItemRatePerKG',
            'gridItemGAmount',
            'gridItemDiscountPercentage',
            'gridItemDiscountPerUnit',
            'gridItemAmountExclTax',
            'gridItemTaxPercentage',
            'gridItemTaxAmount',
            'gridItemAmountInclTax',
            'txtGridRemarks',
            'txtDiscount',
            'txtDiscAmount',
            'txtExpense',
    ...
    let returnInwardViewList = undefined;
    const getReturnInwardDataTable = (returnInwardId = 0, fromDate = "", toDate = "") => {
        if (typeof returnInwardViewList !== 'undefined') {
            returnInwardViewList.destroy();
            $('#returnInwardViewListTbody').empty();
        }
        returnInwardViewList = $("#returnInwardViewList").DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                url: `${baseUrl}/returnInward/getReturnInwardDataTable`,
                type: 'GET',
                data: { 'returnInwardId': returnInwardId, fromDate: fromDate, toDate: toDate },
            ...
            columns: [
                ...
                },
                {
                    data: "vrnoa",
                    name: "vrnoa",
                    className: "text-left returnInwardVoucher"
                },
                {
                    data: "vrdate",
                    name: "vrdate",
                    ...
                    render: function (data, type, row) {
						...
						<div class="btn-group dropleft">
							...
							<div class="dropdown-menu">
								<a class="dropdown-item btnEditPrevVoucher" data-return_inward_id="${row.id}" href="#"><i class='fa fa-edit'></i> Edit Voucher</a>
                            ...
                            <ul class="dropdown-menu dropdown-submenu-left" role="menu">
                                <li class="dropdown-item"><a href="#" class="btnPrint" data-return_inward_id   ="${row.id}">Print Voucher</a></li>
                                <li class="dropdown-item"><a href="#" class="btnPrintA4WithHeader" data-return_inward_id   ="${row.id}"> Print a4 with header</a></li>
                                <li class="dropdown-item"><a href="#" class="btnPrintA4WithOutHeader" data-return_inward_id   ="${row.id}"> Print a4 without header </a></li>
                                <li class="dropdown-item"><a href="#" class="btnPrintB5WithHeader" data-return_inward_id   ="${row.id}"> Print b5 with header</a></li>
                                <li class="dropdown-item"><a href="#" class="btnPrintB5WithOutHeader" data-return_inward_id   ="${row.id}"> Print b5 without header </a></li>
                            </ul>

								<a class="dropdown-item btnDelete" data-return_inward_id="${row.id}" href="#"><i class='fa fa-trash'></i> Delete Voucher</a>
...
`;
        ...
        returnInwardViewList.on('draw', function () {
    ...
    const getPendingSaleOrderRecord = () => {
        ...
        pendingSaleOrderRecordList = $("#pendingSaleOrderRecordList").DataTable({
            ...
            serverSide: true,
            ajax: {
                url: `${baseUrl}/saleOrder/getPendingSOCompareOutward`,
                type: 'GET',
                dataSrc: function (json) {
                    return json.data;
                }
            ...
            columns: [
                ...
                { data: "customerName", name: 'parties.name', className: "customer" },
                { data: "item_des", name: 'items.item_des', className: "itemName" },
                {
                    searchable: false,
                    data: "remaining_qty",
    ...
    var getPendingSaleOrderById = async function (saleOrderId) {
        const response = await makeAjaxRequest('GET', `${baseUrl}/saleOrder/getPendingSaleOrderById`, {
    ...
    var populatePendingSaleOrderData = async function (data) {
        ...
        mainData.party = { pid: mainData.customerId, name: mainData.customerName };
        appendSelect2ValueIfDataExists("accountDropdown", "party", "pid", "name", mainData, { disabled: true });
        mainData.sale_officer = { id: mainData.saleOfficerId, name: mainData.saleOfficerName };
        ...
        const tableAppended = new TableRowAppended('#purchase_table', { 'stockKeepingMethod': stockKeepingMethodId, 'isUnitRate': false, ...propsForTable });
    ...
    var getSaleCommissionPercentage = async function (accountId, saleOfficerId) {
        const response = await makeAjaxRequest('GET', `${baseUrl}/saleOfficerCommissionAllotment/getSaleCommissionPercentage`, {
            accountId: accountId,
    ...
    async function validateInventory(itemId, stockKeepingMethodId, colorCodeId, departmentId, qty, weight, currentDate) {
        const inventoryManager = new InventoryManager({ "voucherType": voucherType, "voucherId": $('#returnInwardHiddenId').val() });
        ...
        bindUI: function () {
            ...
            $('#gridItemShortCodeDropdown').on('change', async function (e) {
                ...
                if (parseNumber(itemPriceListId) == 0) {
                    ...
                    return AlertComponent.getAlertMessage({ title: "Error", message: "Please select item price list", type: "danger" });
                }
            ...
            $('#purchase_table').on('click', '.btnRowEdit', async function (event) {
                ...
                const stockInfo = new StockInformation(editStockKeepingMethodId, { isSale: true, voucherType: voucherType, voucherId: $('#returnInwardHiddenId').val() });
            ...
            $('#saleOrderFilter').on('click', function (e) {
                ...
                getReturnInwardDataTable("", fromDate, toDate);
            });
            ...
            $('body').on('click', '#pendingSaleOrderLookUp .getSaleOrderById', async function (e) {
                e.preventDefault();
            ...
            $('body').on('click', '.btnEditPrevVoucher', function (e) {
                e.preventDefault();
                const returnInwardId = parseNumber($(this).data('return_inward_id'));
                getReturnInwardById(returnInwardId);
            ...
            $('body').on('click', '.btnPrint', function (e) {
                ...
                e.preventDefault();
                Print_Voucher(returnInwardId, settingPrintDefault, 'lg', '');
            });
            ...
            $('body').on('click', '.btnPrintA4WithHeader', function (e) {
                ...
                if ($('#setting_print_default').val() == 5 || $('#setting_print_default').val() == 6) {
                    ...
                    Print_Voucher(returnInwardId, 1, 'lg', "");
                }
            });
            $('body').on('click', '.btnPrintA4WithOutHeader', function (e) {
                ...
                if ($('#setting_print_default').val() == 5 || $('#setting_print_default').val() == 6) {
                    ...
                    Print_Voucher(returnInwardId, 2, 'lg', "");
                }
            });
            $('body').on('click', '.btnPrintB5WithHeader', function (e) {
                ...
                if ($('#setting_print_default').val() == 5 || $('#setting_print_default').val() == 6) {
                    ...
                    Print_Voucher(returnInwardId, 3, 'lg', "");
                }
            });
            $('body').on('click', '.btnPrintB5WithOutHeader', function (e) {
                ...
                if ($('#setting_print_default').val() == 5 || $('#setting_print_default').val() == 6) {
                    ...
                    Print_Voucher(returnInwardId, 4, 'lg', "");
                }
            });
            ...
            $('body').on('click', '.btnPrintASEmail', function (e) {
                ...
                const settingPrintDefault = $('#setting_print_default').val();
                e.preventDefault();
                getSendMail(returnInwardId, settingPrintDefault, 'lg', '', true);
            });
        ...
        initSave: function () {
            ...
            if (!validateSaveFlag) {
                ...
                if (rowsCount > 0) {
                    ...
                    const data = { 'returnInward': JSON.stringify(saveObj.returnInward), 'returnInwardDetail': JSON.stringify(saveObj.returnInwardDetail), 'id': saveObj.id, 'chk_date': $('#chk_date').val(), 'vrdate': $('#current_date').val() };
...
const returnInward = new ReturnInward();
...
$(function () {
    ...
    new DynamicOption('#gridItemPriceListDropdown', {
        requestedUrl: `${baseUrl}/item/itemPrice/getAllPriceList`
    });
    ...
    new DynamicOption('#gridItemColorCodeDropdown', {
        requestedUrl: `${baseUrl}/color/getAllColorCodes`,
